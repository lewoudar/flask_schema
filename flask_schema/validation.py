    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            try:
                ...
                return func(*args, headers=model, **kwargs)
...
def validate_response(
    ...
    This ensures that the response is either dictionary that the
                ...
                if headers_model_class is not None:
                    try:
                        if isinstance(headers, dict):
                            headers_model_value = _convert_headers(
                                headers, headers_model_class
                            )
                    ...
                    if is_dataclass(headers_model_value):
                        ...
                        headers_value = cast(
                            BaseModel, headers_model_value
                        ).model_dump()
                else:
                    headers_value = headers

                return model_value.model_dump(), status, headers_value
            else:
                return result
...
def validate(
    *,
    ...
    This is a shorthand combination of the validate_querystring,
